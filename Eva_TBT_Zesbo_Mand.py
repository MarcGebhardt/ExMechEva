...
Changelog:
    ...
    - 23-06-30: Start und Ende wieder original (s. 23-03-08)
...
def TBT_single(prot_ser, paths, mfile_add=''):
    ...
    DQopt=pd.concat(Evac.Diff_Quot(messu.loc(axis=1)[dic_used_Strain], messu.loc(axis=1)['Stress'],
                            _opts['OPT_YM_Determination_refinement'][-2],
                            _opts['OPT_YM_Determination_refinement'][-1]), axis=1)
    ...
    if _opts['OPT_DIC']:
        ax1.plot(messu.loc[:VIP_messu[Vpltend]][dic_used_Strain],
                 messu.loc[:VIP_messu[Vpltend]]['Stress'], 'b--',label='opt')
    ...
    for x in VIP_messu[:Vpltend].index:
        ...
        ax1.annotate('%s' % x, xy=(a.iloc[j],b.iloc[j]), xycoords='data', color = 'g',
                      xytext=c, ha="center", va="center", textcoords='offset points')
    if _opts['OPT_DIC']:
        a,b=messu.loc[VIP_dicu[:Vpltend],dic_used_Strain],messu.Stress[VIP_dicu[:Vpltend]]
        j=np.int64(-1)
        ax1.plot(a, b, 'x', color='orange')
        j=np.int64(-1)
        for x in VIP_dicu[:Vpltend].index:
            j+=1
            if j%2: c=(6,-6)
            else:   c=(-6,6)
            ax1.annotate('%s' % x, xy=(a.iloc[j],b.iloc[j]), xycoords='data', color='orange',
                          xytext=c, ha="center", va="center", textcoords='offset points')
...
def main():
    ...
    protpaths.loc['S1','path_con']  = "1_Schwein/Messdaten/kon/"
    protpaths.loc['S1','path_dic']  = "1_Schwein/Messdaten/opt/"
    protpaths.loc['S1','path_eva1'] = "1_Schwein/Auswertung/"
    protpaths.loc['S1','path_eva2'] = "Single/"
    
    protpaths.loc[:,'path_main']    = "F:/ZESBO/Kiefer/"
    ...
    if option == 'single':
        ...
        ser='S1'
        out_path = combpaths.loc[ser,'out']+ser+"CBPig_TBT-Summary"
        ...
        out_path="F:/ZESBO/Kiefer/1_Schwein/Auswertung/Complete/CBPig_TBT-Summary"
        ...
        out_path="F:/ZESBO/Kiefer/1_Schwein/Auswertung/Complete/CBPig_TBT-Summary_all"
        packpaths = combpaths[['prot','out']]
        packpaths.columns=packpaths.columns.str.replace('out','hdf')
