...
def Point_df_from_lin(x, steps, coords=['x','y'], coords_set='x',
    ...
    if coords is None:
        ...
        mit = pd.Index(xt).map(lambda x: Eva_common.round_to_sigdig(x,_significant_digits_fpd))
    ...
    **pwargs : dict or pandas.Series
    ...
    for step in s_range:
        if step == s_range[0]:
            ...
            # step_bf = Eva_common.pd_slice_index(s_range,step-1,option='list')
...
def stress_perF(x, func_MoI, func_thick, xmin,xmax, evopt=1/2, Test_type='TPB'):
    ...
    if isinstance(x,float):
        ...
        # round float point division
        ind=pd.Index(x).map(lambda x: Eva_common.round_to_sigdig(x,_significant_digits_fpd))
...
#     step_range=Eva_common.pd_combine_index(stress_mid_ser, strain_mid_ser)
...
#         YM, YM_abs, YM_Rquad, YM_fit = Eva_common.YM_sigeps_lin(stress_mid_ser,
#                                                                 strain_mid_ser, **kws)
...
def YM_eva_method_A(stress_mid_ser,
    ...
    Children of Eva_common.YM_eva_com_sel.
    ...
    **kws : dict
    ...
    """
    YM = Eva_common.YM_eva_com_sel(stress_ser=stress_mid_ser,
...
def YM_eva_method_B(stress_mid_ser,
    ...
    **kws : dict
    ...
    if option == "Points":
        ...
        # step_range=Eva_common.pd_combine_index(stress_mid_ser, P_df)
        step_range=Eva_common.pd_combine_index(stress_mid_ser, P_df.index)
        ...
        step_range=Eva_common.pd_combine_index(stress_mid_ser, w_params)
    ...
    if det_opt=="incremental":
        ...
        if stress_mid_ser.isna().all() or strain_ser.isna().all():
            ...
            YM, YM_abs, YM_Rquad, YM_fit = Eva_common.YM_sigeps_lin(stress_mid_ser, strain_ser, **kws)
...
def YM_eva_method_E(Force_ser, length,
    ...
    def MaxMinboSer(po,oths,k='Max',axis=1):
        gax = Eva_common.pd_axischange(axis)
    ...
    step_range=Eva_common.pd_combine_index(Force_ser, params_curve)
    ...
    if opt_det in ['all', 'range']:
        if opt_g == 'length':
            cols = Eva_common.pd_slice_index(E_to_x.columns,
                                             [xlin.min()*opt_g_lim, 
                                              xlin.max()*opt_g_lim])
...
def YM_eva_method_G(Force_ser,
    ...
    """
    step_range = Eva_common.pd_combine_index(Force_ser, w_params)
...
def YM_eva_method_F(c_func, c_params,
    ...
    step_range = Eva_common.pd_combine_index(Force_ser, c_params)    
...
def YM_eva_method_D_bend_df(Length, I_func, n=100, E=1, F=1):
    ...
    _significant_digits_fpd=12
    x=x.map(lambda x: Eva_common.round_to_sigdig(x,_significant_digits_fpd))
...
def YM_eva_method_D(P_df, Force_ser,
    ...
    # step_range = Eva_common.pd_combine_index(Force_ser, P_df)
    step_range = Eva_common.pd_combine_index(Force_ser, 
                                             P_df.loc[np.invert(P_df.isna().all(axis=1))].index)
    
    if not rel_steps is None: 
        step_range = Eva_common.pd_combine_index(rel_steps, step_range)
    
    if weighted:
        if (type(wkwargs) is pd.core.series.Series):
             step_range = Eva_common.pd_combine_index(step_range,
                                                      wkwargs)
...
def YM_check_many_with_method_D(E_dict, F, Length, I_func,
    ...
    step_range = Eva_common.pd_combine_index(F, w_params)
    if not rel_steps is None: 
        step_range = Eva_common.pd_combine_index(rel_steps, step_range)
    xlin=np.linspace(-Length/2,Length/2,n+1)
